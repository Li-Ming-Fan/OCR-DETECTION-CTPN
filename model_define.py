...
def conv_feat_layers(inputs, width, detect, training):
    ...
    layer_params = [ [  64, (3,3), (1,1),  'same', True, True, 'conv1'], 
                     ...
                     [ 256, (2,2), (2,2), 'valid', True, True, 'pool3'] ] # for pool
                     
    layers_detect = [ [512, (3,3), (1,1),  'same', True, True, 'feat_d'] ] # for feat
                     
    layers_recog = [ [ 256, (3,1), (1,1), 'valid', True, True, 'conv_r1'],
                     [ 256, (3,1), (1,1), 'valid', True, True, 'conv_r2'],
                     [ 256, (3,1), (1,1), 'valid', True, True, 'conv_r3'],
                     [ 512, (2,1), (1,1), 'valid', True, True, 'feat_r' ] ] # for feat
...
#   
def detect_results(rnn_feat, num_anchors):
    #
    weight_initializer = tf.contrib.layers.variance_scaling_initializer()
    bias_initializer = tf.constant_initializer(value=0.0)
    #
...
#
def recog_results(rnn_feat, num_classes):
    #
    weight_initializer = tf.contrib.layers.variance_scaling_initializer()
    bias_initializer = tf.constant_initializer(value=0.0)
    #
    rnn_logits = tf.layers.dense(rnn_feat, num_classes,
                                 activation = tf.nn.sigmoid,
                                 kernel_initializer = weight_initializer,
                                 bias_initializer = bias_initializer,
                                 name = 'recog_logits')
