...
class ModelDetect():
    ...
    def predict(self, sess, img_file, out_dir = './results_prediction'):
        ...
        model_data_detect.getImageAndTargets(img_file, meta.anchor_heights)
        #
        img_size = model_data_detect.getImageSize(img_file) # width, height
        #
        ...
        with self.graph.as_default():              
            ...
            #
            # trans
            text_bbox = model_data_detect.transResults(r_cls, r_ver, r_hor, \
                                                       meta.anchor_heights, meta.threshold)
            #
            model_data_detect.drawTextBox(file_target, text_bbox)
            #
    ...
    def z_define_graph_all(self, graph, train = True): # learn.ModeKeys.TRAIN  INFER
        ...
        with graph.as_default():
            ...
            #
            conv_feat, sequence_length = model_define.conv_feat_layers(x, w, True, train)   # train
            rnn_feat = model_define.rnn_feat_layers(conv_feat, sequence_length)
            rnn_cls, rnn_ver, rnn_hor = model_define.detect_results(rnn_feat, len(meta.anchor_heights))
            #
    ...
    def train_and_valid(self):
        #
        # get training images
        list_images_train = model_data_detect.getFilesInDirect(meta.dir_images_train, meta.str_dot_img_ext)
        #      
        # model save-path
